...
class Tree(object):
    ...
    # the lists below can be given as a space-separated string of names instead of a list
    _ignore_ = []               # list of non-terminals that should be ignored during rewriting (pruned from the tree)
    _reduce_ = []               # list of non-terminals that should be replaced with a list of their children
    _compact_ = []              # list of non-terminals that should be replaced with their child if there is exactly 1 child AFTER rewriting of all children
